...
public class BeerTower extends GameObject implements Clickable
{
	...
	private int x = 695;
	private int y = 390;
	private double cooldownTime = .2; //time that we're not allowed to fire.
	 ...
	 */
	public BeerTower(Control control, State state)
	{
		this.state = state;
	...
	public void update(double elapsedTime) 
	{
		...
		else
		{
			...
			if(cooldownTime<=0)
			{
				...
				{
					int vx = 2*(int)(e.getLocation().getX()-x);
					int vy = 2*(int)(e.getLocation().getY()-(y-20));
					state.addGameObject(new FlyingBeer(control,state,x,y,vx,vy));
					cooldownTime += .2;
				}
				
			}
		}
		System.out.println(state.findNearestEnemy(x, y));
 ...
 */
	...
	{
		BufferedImage saltTower = control.getImage("beer.png");
