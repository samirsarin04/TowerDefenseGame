	...
	public class BeerButton extends GameObject implements Clickable
	{
		private Control control;
		private State state;
		private int x,y,width,height;
		
		public BeerButton(Control control, State state)
		{
			isVisible = true;
			isExpired = false;
		...
		public void draw(Graphics g) 
		{
			...
			if(state.getMoney() >= state.getBeerCost())
			{
			...
			g.drawImage(control.getImage("beer.png"),x+10,y+10,width - 20, height - 20,null); //Draw a salt image on the button.
			if(state.getMoney()>=state.getBeerCost())
			{
			...
			g.drawString("400", x+4, y+height-4);
		...
		if(control.getMouseX()>=x&control.getMouseX()<=x+width&&control.getMouseY()>y&&control.getMouseY()<=y+height) //Check to see if the click is in the SaltButton area.
		{
			...
			state.currentFrameGameObjects.add(new BeerTower(control,state));//Add a new salt tower to the current game object list.
			state.setMoney(-state.getBeerCost());
